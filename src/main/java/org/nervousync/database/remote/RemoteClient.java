...
import org.nervousync.database.query.result.ResultList;
...
public interface RemoteClient {
	...
	/**
	 * <h3 class="en-US">Create a save point</h3>
	 * <h3 class="zh-CN">创建保存点</h3>
	 *
	 * @param txCode <span class="en-US">Transactional identify code</span>
	 *               <span class="zh-CN">事务识别代码</span>
	 * @param name   <span class="en-US">Savepoint name</span>
	 *               <span class="zh-CN">保存点名称</span>
	 * @return <span class="en-US">Execute result</span>
	 * <span class="zh-CN">执行结果</span>
	 */
	@POST
	@Path("/savepoint/{txCode}/{name}")
	@Produces(MediaType.TEXT_PLAIN)
	@WebMethod
	Boolean createSavepoint(@WebParam @PathParam("txCode") final Long txCode,
	                        @WebParam @PathParam("name") final String name);

	/**
	 * <h3 class="en-US">Release a save point</h3>
	 * <h3 class="zh-CN">释放保存点</h3>
	 *
	 * @param txCode <span class="en-US">Transactional identify code</span>
	 *               <span class="zh-CN">事务识别代码</span>
	 * @param name   <span class="en-US">Savepoint name</span>
	...
	@Produces(MediaType.TEXT_PLAIN)
	@WebMethod
	Boolean releaseSavepoint(@WebParam @PathParam("txCode") final Long txCode,
	                         @WebParam @PathParam("name") final String name);
	 ...
	 */
	@GET
	@Path("/transactional/rollback/{txCode}/{name}")
	@Produces(MediaType.TEXT_PLAIN)
	@WebMethod
	Boolean rollback(@WebParam @PathParam("txCode") final Long txCode,
	                 @WebParam @PathParam("name") final String name);
	 ...
	 * @param txCode        <span class="en-US">Transactional identify code</span>
	 *                      <span class="zh-CN">事务识别代码</span>
	 * @param identifyCodes <span class="en-US">Identify information of the return data</span>
	 *                      <span class="zh-CN">返回数据的标识信息</span>
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@WebMethod
	ResultList insert(@Nonnull @WebParam @PathParam("databaseName") final String databaseName,
	                  @Nonnull @WebParam @PathParam("tableName") final String tableName,
	                  @WebParam @QueryParam("txCode") final Long txCode,
	                  @WebParam @QueryParam("identify") final Object identifyCodes,
	 ...
	 * @param identifyCodes <span class="en-US">Identify information of the return data</span>
	 *                      <span class="zh-CN">返回数据的标识信息</span>
	 * @param queryInfo     <span class="en-US">Data query information</span>
	 *                      <span class="zh-CN">数据查询信息</span>
	 * @return <span class="en-US">Primary key values mapping table generated by database</span>
	 * <span class="zh-CN">数据库生成的主键值数据映射表</span>
	 */
	@POST
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@WebMethod
	Long update(@Nonnull @WebParam @PathParam("databaseName") final String databaseName,
	            @Nonnull @WebParam @PathParam("tableName") final String tableName,
	            @WebParam @QueryParam("txCode") final long txCode,
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@WebMethod
	Long delete(@Nonnull @WebParam @PathParam("databaseName") final String databaseName,
	            @Nonnull @WebParam @PathParam("tableName") final String tableName,
	            @WebParam @QueryParam("txCode") final long txCode,
	            @Nonnull @WebParam @QueryParam("filter")
	            @DataTransfer(adapter = JsonBeanAdapter.class,
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@WebMethod
	ResultList query(@Nonnull @WebParam @QueryParam("query")
	                 @DataTransfer(adapter = JsonBeanAdapter.class,
			                 initParam = "org.nervousync.database.query.QueryInfo") final QueryInfo queryInfo);
